...
export default function ReplayBar({
  ...
  return (
    <div
      className={`absolute bottom-3.5 left-3.5 z-[1000] rounded-card border border-border bg-white/95 px-4 py-2.5 shadow-card backdrop-blur-md transition-[right] ${
        panelOpen ? "right-[362px]" : "right-3.5"
      }`}
    >
      <div className="flex items-center gap-3">
        <button
          ...
          className="h-[38px] w-[38px] shrink-0 rounded-full bg-accent text-[13px] text-white disabled:opacity-40"
        >
          {playing ? "❚❚" : "▶"}
        </button>
        ...
        <select
          ...
          title="Velocidad"
          className="shrink-0 rounded-[10px] border border-border bg-surface px-2 py-1.5 font-mono text-xs"
        >
